...
public class DataUtilitiesTest extends TestCase {
	...
	public void testPostiveAndFinalColumnTotal() {
		...
		//assertEquals(4.0, total, 0.0000001d);
	}
	...
	public void testNegativeColumnTotal() {
		DefaultKeyedValues2D testValues = new DefaultKeyedValues2D();
		values2D = testValues;
		testValues.addValue(-0.0, -3.0, -1.0);
		testValues.addValue(-3.0, -1.0, -0.0);
		
		double total = DataUtilities.calculateColumnTotal(testValues, 0);
		//assertEquals(-7.0, total, 0.0000001d);
	}
	...
	// calulateRowTotal Tests
		...
		public void testPostiveAndFinalRowTotal() {
			...
			values2D = testValues;
			testValues.addValue(0.0, 3.0, 1.0);
			testValues.addValue(3.0, 1.0, 2.0);
			assertEquals("Did not return expected result", 2.0, DataUtilities.calculateRowTotal(values2D, 1), 0.0000001d);
		}
		...
		public void testNegativeRowTotal() {
			DefaultKeyedValues2D testValues = new DefaultKeyedValues2D();
			values2D = testValues;
			testValues.addValue(-1.0, -2.0, -0.0);
			testValues.addValue(-4.0, -2.0, -1.0);
			assertEquals("Did not return expected result", 0.0, DataUtilities.calculateRowTotal(values2D, 0), 0.0000001d);
		}
		...
		public void testNegativeAndFinalRowTotal() {
			DefaultKeyedValues2D testValues = new DefaultKeyedValues2D();
			values2D = testValues;
			testValues.addValue(-5.0, -2.0, -3.0);
			testValues.addValue(-4.0, -2.0, -1.0);
			assertEquals("Did not return expected result", -3.0, DataUtilities.calculateRowTotal(values2D, 0), 0.0000001d);
		}
		
		@Test
		public void testEmptyRowTotal() {
			DefaultKeyedValues2D testValues = new DefaultKeyedValues2D();
			assertEquals("Return 0 for empty data", 0.0, DataUtilities.calculateRowTotal(testValues, 0), 0.0000001d);
		}
	...
	public void testGetCumulativePercentagesEmpty() {
		...
		assertNotNull(cumulativePercentages);
